...
add_filter('rmc/crumb' , function($crumb , $settings){
	...
	if(get_post_type() =='news-room'):
		if(!isset($crumb['post'])): $crumb['post'] = array(); endif;

		if(is_single()):
			$crumb['post'][] = '<a href="' . home_url('news-room') . '">Newsroom</a><em class="crumb-sep"> / </em><span class=\"current-crumb-item\">'.get_the_title($post->ID).'</span>';
		endif;
		if(is_category() || is_archive() || is_home()):
			if(is_category()):
				$title = single_cat_title( '', false );
				$crumb['post'][] = "<span class=\"current-crumb-item\">Newsroom</span>";
			elseif(is_archive()):
				$title = (isset($wp_query->query['monthnum'])) ? get_the_time('F Y') : get_the_time('Y') ;
				$crumb['post'][] = "<span class=\"current-crumb-item\">Newsroom</span>";
			else:
...
add_filter('rmc/crumb' , function($crumb , $settings){
	...
	if(in_array(get_post_type() , array('rmg-case','rmg-category'))):
		...
		if(is_archive()):
			$crumb['gallery'][] = '<span class="current-crumb-item">Gallery</span>';
		else:
