        ...
        function countLikes() {
            ...
            function readPageforLikablePosts() {
                for (let i = 0; i < engageBar.length; i++) {
                    var likeID = engageBar[i].dataset.id,
                        ...
                        likeButton = engageBar[i].querySelector(
                            ".the-like-button i"
                        ),
                        likeVisualCount = engageBar[i].querySelector(
                            ".the-like-counter"
                        ),
                        theFire = engageBar[i].querySelector(".the-fire"),
                        hasVoted = false;

                    const thePost = new mapPostData(
                        likeID,
                        likeCount,
                        likeURL,
                        likeButton,
            ...
            function toggleCookie(likeID) {
                ...
                if (
                    !document.cookie
                        .split(";")
                        .filter((item) => item.trim().startsWith(likeID)).length
                ) {
            ...
            function mapPostData(id, count, url, button, visualcount, thefire) {
                ...
                function postLikedActions(event) {
                    ...
                    if (
                        !document.cookie
                            .split(";")
                            .filter((item) => item.trim().startsWith(id)).length
                    ) {
    ...
    if (myVideoWrap) {
        ...
        function createVideoMarkup(item) {
            let videoMarkup =
                ...
                theme_path +
                '/images/slide-1.jpg" class="bgvid"><source src="' +
                theme_path +
                "/images/" +
                item +
...
function showSocialFeeds() {
    ...
    var feedURL =
        "https://sandiegohousemusic.com/wordpress/wp-content/themes/sandiegohousemusic/";
